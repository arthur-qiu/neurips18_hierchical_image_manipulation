...
class Pix2PixHDModel_condImgAdv(BaseModel):
    ...
    def mult_interp_attack(self, label, label1, label2, inst, inst1, image, mask_in, mask_out, mask_target, id):
        ...
        for i in range(20):
            ...
            print('acc: %.3f' % (((pred * mask_target.squeeze(1).long() == target_labels * mask_target.squeeze(1).long()).cpu().data.numpy().sum() - 256 * 256 + mask_target.squeeze(1).cpu().data.numpy().sum()) / torch.sum(mask_target.squeeze(1).cpu().data.numpy().sum())))
            print('iteration %d loss %.3f' % (int(i), hou_loss.cpu().data.numpy()))
            hou_loss.backward()
            alpha_optimizer.step()
        # semantic attack ends
